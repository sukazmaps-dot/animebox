  ...
  try {
    ...
    const [casesResult, restrictionsResult] = await Promise.all([
      admin
        ...
        .select(
          'id,case_number,claimant_name,claimant_company,claimant_email,claimant_role,work_title,rights_description,authority_statement,signature,status,created_at,updated_at,reviewed_at',
        )
