  ...
  return (
    <section className={styles.dashboard} aria-label="AnimeBox Recommendation Analytics">
      <header className={styles.header}>
        <div><span className={styles.eyebrow}>TASTE GRAPH · V2</span><h1>Recommendations</h1><p>От показа карточки до реального старта и завершения серии.</p></div>
      ...
      {dashboard && k && <>
        <div className={styles.kpis}>
          ...
          <article><span>Play → Completed</span><strong>{pct(k.startedToCompletedPct)}</strong><small>{num(k.completed)} завершений</small></article>
          <article><span>Dismiss rate</span><strong>{pct(k.dismissRatePct)}</strong><small>{num(k.dismissed)} скрыто</small></article>
          <article><span>Planned</span><strong>{num(k.planned)}</strong><small>добавлено в планы</small></article>
        ...
        <section className={styles.panel}>
          ...
          <div className={styles.tableWrap}><table className={styles.table}><thead><tr><th>Source</th><th>Impressions</th><th>Clicks</th><th>CTR</th><th>Play</th><th>Completed</th></tr></thead><tbody>{dashboard.sources.map((source) => <tr key={source.source}><td>{source.source}</td><td>{num(source.impressions)}</td><td>{num(source.clicks)}</td><td>{pct(source.ctrPct)}</td><td>{num(source.started)}</td><td>{num(source.completed)}</td></tr>)}</tbody></table></div>
        </section>
        <section className={styles.panel}>
