    ...
    <section className={styles.dashboard} aria-label="AnimeBox Recommendation Analytics">
      ...
      {dashboard && k && <>
        ...
        <section className={styles.panel}>
          ...
          <div className={styles.tableWrap}><table className={styles.table}><thead><tr><th>Row</th><th>Impressions</th><th>CTR</th><th>Play</th><th>15m</th><th>Dismiss</th><th>End</th><th>Loads</th><th>Added</th><th>Fill</th><th>DOM / Rail</th><th>Pages</th><th>Errors</th></tr></thead><tbody>{dashboard.rows.map((row) => <tr key={row.rowId}><td>{row.rowId}</td><td>{num(row.impressions)}</td><td>{pct(row.ctrPct)}</td><td>{num(row.started)}</td><td>{num(row.watch15m)}</td><td>{pct(row.dismissRatePct)}</td><td>{num(row.endReached)}</td><td>{num(row.loadRequests)}</td><td>{num(row.loadAdded)}</td><td>{pct(row.loadFillPct)}</td><td>{num(row.maxRenderedItems)} / {num(row.maxRailItems)}</td><td>{num(row.pagesScanned)}</td><td>{num(row.loadErrors)}</td></tr>)}</tbody></table></div>
