'use client';

import type { LibraryStatus } from '@/lib/community-client';

export type TrackerSnapshot = {
  stats: Record<LibraryStatus, number>;
  library: { anime_id: number; title: string; status: LibraryStatus }[];
};

type CacheEntry = { data: TrackerSnapshot; expiresAt: number };
const TTL_MS = 60_000;
const PREFIX = 'animebox:tracker:v3:';
const memory = new Map<string, CacheEntry>();
const inflight = new Map<string, Promise<TrackerSnapshot>>();

function readSession(userId: string): CacheEntry | null {
  if (typeof window === 'undefined') return null;
  try {
    const raw = window.sessionStorage.getItem(`${PREFIX}${userId}`);
    if (!raw) return null;
    const value = JSON.parse(raw) as CacheEntry;
    if (!value?.data || value.expiresAt <= Date.now()) {
      window.sessionStorage.removeItem(`${PREFIX}${userId}`);
      return null;
    }
    return value;
  } catch {
    return null;
  }
}

function save(userId: string, data: TrackerSnapshot) {
  const entry = { data, expiresAt: Date.now() + TTL_MS };
  memory.set(userId, entry);
  if (typeof window === 'undefined') return;
  try {
    window.sessionStorage.setItem(`${PREFIX}${userId}`, JSON.stringify(entry));
  } catch {
    // Ignore storage failures.
  }
}

export function peekTrackerSnapshot(userId: string | null | undefined) {
  if (!userId) return null;
  const cached = memory.get(userId);
  if (cached && cached.expiresAt > Date.now()) return cached.data;
  if (cached) memory.delete(userId);
  const session = readSession(userId);
  if (session) {
    memory.set(userId, session);
    return session.data;
  }
  return null;
}

export function seedTrackerSnapshot(userId: string, data: TrackerSnapshot) {
  save(userId, data);
}

export function invalidateTrackerSnapshot(userId: string | null | undefined) {
  if (!userId) return;
  memory.delete(userId);
  inflight.delete(userId);
  if (typeof window === 'undefined') return;
  try {
    window.sessionStorage.removeItem(`${PREFIX}${userId}`);
  } catch {
    // Ignore storage failures.
  }
}

export async function getTrackerSnapshot(userId: string, force = false) {
  if (!force) {
    const cached = peekTrackerSnapshot(userId);
    if (cached) return cached;
    const pending = inflight.get(userId);
    if (pending) return pending;
  }

  const request = fetch('/api/community/tracker', { cache: 'no-store' }).then(
    async (response) => {
      const data = (await response.json()) as TrackerSnapshot & { error?: string };
      if (!response.ok) throw new Error(data.error || 'Не удалось загрузить библиотеку.');
      save(userId, data);
      return data;
    },
  );

  inflight.set(userId, request);
  try {
    return await request;
  } finally {
    if (inflight.get(userId) === request) inflight.delete(userId);
  }
}
